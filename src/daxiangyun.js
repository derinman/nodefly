...
const postMessage = (iframeWindow, type, data) => {
  iframeWindow.contentWindow.postMessage(
    ...
    process.env.REACT_APP_VIEWER_SERVER_HOST
  );
};

//onCam Callback to update 2D top, left
...
// 移動至選定實體位置
function msgGotoEntity(iframeWindow, bbox) {
  postMessage(iframeWindow.current, "MSG_GOTO_ENTITY", { bbox: bbox });
}
...
export { msgViewSnapShot, msgSetToolbarItems, onCamChange, msgGotoEntity, msgProjectPoint };
